...
func (r *AuthRepository) DeleteUserByID(ctx context.Context, userID uuid.UUID) error {
...
func (r *AuthRepository) CheckUserExists(ctx context.Context, emailOrPhone string) (uuid.UUID, error) {
	var exists bool
...
func (r *AuthRepository) GetAuthCodeByRecipient(ctx context.Context, recipient string) (*models.AuthCode, error) {
	...
	if authCode.AuthCodeID == uuid.Nil {
		return nil, err
	}
	return &authCode, err
}

func (r *AuthRepository) GetAuthCodeByCodeAndUserID(ctx context.Context, code string, userID uuid.UUID) (*models.AuthCode, error) {
	var authCode models.AuthCode
	err := r.db.GetContext(ctx, &authCode, "SELECT * FROM auth.auth_codes WHERE code = $1 AND user_id = $2", code, userID)
	return &authCode, err
}

func (r *AuthRepository) GetAuthCodeByID(ctx context.Context, codeID uuid.UUID) (*models.AuthCode, error) {
	var authCode models.AuthCode
	err := r.db.GetContext(ctx, &authCode, "SELECT * FROM auth.auth_codes WHERE code_id = $1", codeID)
	return &authCode, err
}

func (r *AuthRepository) DeleteAuthCodeByID(ctx context.Context, codeID uuid.UUID) error {
...
func (r *AuthRepository) CreateSession(ctx context.Context, session *models.Session) error {
	...
	`, session.Token, session.UserID, session.ExpiresAt).StructScan(session)
	return err
}

func (r *AuthRepository) GetSessionByUserID(ctx context.Context, userID uuid.UUID) (*models.Session, error) {
	var session models.Session
	err := r.db.GetContext(ctx, &session, "SELECT * FROM auth.sessions WHERE user_id = $1 ORDER BY sessions.expires_at DESC LIMIT 1", userID)
	return &session, err
}

func (r *AuthRepository) DeleteSessionByID(ctx context.Context, sessionID uuid.UUID) error {
